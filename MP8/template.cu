...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  //@@ insert spmv kernel for jds format
   ...
   float spmv = 0;
    if(temp<dim) 
    {    
    for (int i = 0; i < matRows[temp]; ++i)
    {
      spmv += matData[matColStart[i]+temp]*vec[matCols[matColStart[i]+temp]];
    }
      out[matRowPerm[temp]] = spmv;
  }
}
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  size_t w = ceil(dim/(float)1024);
  spmvJDSKernel<<<w, 1024>>>(out, matColStart, matCols, matRowPerm, matRows, matData, vec, dim);
